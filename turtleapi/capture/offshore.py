...
def mini_query_offshore(data):
    ...
    if filters['capture_date'] is not None and filters['metadata_id'] is None: # Overwrite metadata_id only if it doesn't exist and we have a metadata_date asked
        filters['metadata_ids'] = db.session.query(OffshoreMetadata.metadata_id).filter(OffshoreMetadata.capture_date == filters['capture_date']).all()
    ...
    result = db.session.query(Metadata.metadata_id, OffshoreEncounter.encounter_id, Turtle.turtle_id, Turtle.species).filter(*queries, Encounter.metadata_id==Metadata.metadata_id, Turtle.turtle_id==Encounter.turtle_id).order_by(OffshoreEncounter.encounter_id.desc()).all() # returns list of result objects
