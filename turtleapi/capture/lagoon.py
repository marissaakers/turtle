...
def mini_query_lagoon(data):
    ...
    result = db.session.query(Metadata.metadata_id, LagoonEncounter.encounter_id, LagoonEncounter.encounter_date, Turtle.turtle_id, Turtle.species).filter(*queries, Encounter.metadata_id==Metadata.metadata_id, Turtle.turtle_id==Encounter.turtle_id).order_by(LagoonEncounter.encounter_date.desc()).all() # returns list of result objects
